...
def list_watchlist(
    ...
    """Every active watchlist entry."""
    return [
        WatchlistEntryModel(
            entry_id=e.entry_id, entity_type=str(e.entity_type), entity_value=e.entity_value,
            reason=e.reason, added_by=e.added_by, active=e.active,
        )
...
def add_watchlist(
    ...
    return WatchlistEntryModel(
        entry_id=entry.entry_id, entity_type=str(entry.entity_type),
        entity_value=entry.entity_value, reason=entry.reason,
        added_by=entry.added_by, active=entry.active,
    )
