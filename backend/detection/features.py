"""
Feature Extraction — the 28-dim ML feature vector (`FeatureExtractor.
extract_all`), graph-derived (in/out degree, pagerank, betweenness from the
`GraphStore`) + transaction-stats features (velocity, dormancy days,
round-number ratio, etc.). Feeds `AnomalyDetector` (IsolationForest) and
`FraudClassifier` (XGBoost) directly (`backend/detection/scoring/
ensemble.py`).

Ported from `archive/fund-flow-tracker/services/detection/features.py`
essentially unchanged — including its documented proxies (`clustering_coeff`
hardcoded to 0.0; `channel_entropy`/`amount_concentration` using cheaper
approximations rather than the archive's own unused "real" helper
functions; `velocity_1hour` actually meaning max-daily-count) — these are
confirmed proxies, not bugs, per ROADMAP Phase 3's "behavioral fidelity over
cleanup" instruction. The archive's own module docstring claims "30
features"; the actual `_FEATURE_COLS` list has always had 28 entries — that
stale count is not repeated here.

Do not conflate this with the separate 16-dim RL context vector
(`backend/detection/rl/bandit.py::LinUCBAgent.build_context`) — that one is
a downstream summary built from this ensemble's *outputs*, not from graph/
...
class FeatureExtractor:
    """Extract the 28-dim feature vector per account from graph + transaction data."""
    ...
    def extract_all(self) -> pd.DataFrame:
        ...
        logger.info("  |- Features: done — %d features x %d accounts (%.1fs)",
