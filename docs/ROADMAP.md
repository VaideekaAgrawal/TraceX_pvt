...
**Scope (checklist):**
- [x] **LLM gateway** — provider-abstracted (OpenRouter now, self-host swap later), with ret/timeout/caching. `foundation/llm_gateway.py`: `generate_completion` (drop-in for the old `call_openrouter`), hand-rolled retry (3 attempts, exponential backoff, `ProviderError.retryable` classification), `OpenRouterProvider`. No second prompt cache — the existing `ai_interactions` fact-keyed cache already satisfies this.
- [x] **PII redaction/tokenization middleware** — pseudonymize identities before egress, re-hydrate on return. `foundation/pii_redaction.py`: `redact_facts`/`rehydrate_text`, request-scoped `token_map` (never persisted), dedup-by-value, widens `db/pii.py`'s column allow-map to cover `account_id`/`customer_id` per `SYSTEM_DEVELOPMENT_PLAN.md` §9.4.
- [x] **Tool layer** — a fixed catalog of retrievable/computed tools scoped to a single case ID; never free-form DB/graph queries. `orchestration/tools/` (`registry.py`, `invoker.py`, `catalog.py`): 9 tools (`similar_cases`, `path_recommendation_facts`, `relationship_graph`, `ego_graph`, `network_risk`, `previous_alerts_summary`, `account_transaction_stats`, `customer_profile`, `search_transactions`), each a thin wrapper over an existing Phase 5-7 `investigation/*` function; `ToolInvoker` binds `case_id` at construction (structurally unoverridable — a caller-passed `case_id` kwarg collides with the positional bind and raises `TypeError`).
- [x] **Guardrail middleware** — sanitize attacker-controllable free text (narration, declared purpose) before it enters any prompt; enforce case-scoping via Phase 2 data-scoping. `foundation/guardrails.py`: `sanitize_free_text` (control-char strip, non-exhaustive injection-phrase neutralization via visible `⟦...⟧` marking, length cap+truncation, untrusted-data delimiter wrap). No live caller yet (same precedent as Phase 7's `path_facts.py`) — case-scoping enforcement lives in the tool-layer wrappers above, which is where a future free-text-reading tool would sit.
- [x] **AI-action audit hook** — every AI call + tools used + facts returned logged to the Phase 4 audit trail. `orchestration/llm_client.py::generate_and_persist_explanation` gains `tools_called`/`redacted` keyword params (both defaulted, existing callers unaffected), threaded into `AiInteractionRepository.create(...)` in place of the previous hardcoded `redacted=False`.
- [x] **`value_hash` HMAC fix** (user-directed addition to this phase's scope, not in the original checklist text): `investigation/relationship_discovery.py`'s `_value_hash` now HMAC-SHA256-keyed by `Settings.pii_hmac_secret` (`foundation/hashing.py`) instead of an unsalted `hashlib.sha256` — fixes the low-entropy-PII brute-force exposure flagged in Phase 7's code review. One-time reconciliation via `RelationshipRepository.clear_all` + `scripts/reconcile_relationship_hashes.py` (destructive, requires `--yes`), since `Relationship` rows are immutable-by-design.
**Explicitly out of scope:** the agents themselves (Phases 9–10).
**Reference:** §5 (AI Guardrails), decision 3 & 4.
**Verification (live, real data, 2026-07-13):** Full pipeline re-run against the real ingested dataset (166,207 customers / 518,573 accounts / 8,002 transactions) — ingest → train (`ensemble_v1`) → detection pipeline (2,740 alerts → 20 auto-created cases) → `generate_demo_data.py` (200 KYC customers, 5,382 relationships via the new HMAC-keyed discovery, 50 historical cases, 7 golden scenarios). `scripts/reconcile_relationship_hashes.py --yes` run against that same DB: cleared 5,382 rows, rediscovered 6,127 under the new hash (same 213-customer candidate pool gate as Phase 7, not the full 166,207 — the count increase vs. the original 5,382 is explained by `demo_data/seed.py`'s fixed stage order, not a Phase 8 regression: `relationship_discovery` runs as stage 3 of 5, before `golden_scenarios` (stage 5) adds its own PAN-bearing customers to the candidate pool — the reconcile script ran after all 5 stages completed, so it saw the fuller pool). `scripts/verify_ai_substrate.py` run against a real pipeline-generated case (`CASE-20260713-024DF0E0`): `ToolInvoker` called `similar_cases`/`path_recommendation_facts`/`network_risk` in sequence (5 similar cases found, network risk score 32 lazy-computed and persisted), `tools_called` accumulated correctly across all 3; `sanitize_free_text` correctly neutralized an injection-phrase sample and truncated it at 500 chars; `redact_facts`/`rehydrate_text` round-tripped correctly with dedup; `generate_completion` via an injected `FakeProvider` (no `OPENROUTER_API_KEY` configured) returned deterministically; `generate_and_persist_explanation(..., tools_called=..., redacted=True)` persisted a real `AiInteraction` row (`id=1`) — re-read from the DB confirming `redacted=True` and populated `tools_called`, which no prior code path in this repo had ever produced. Full local test suite (488 tests post-code-review-fixes, 97% coverage, ~14m54s) + `ruff check .` + `mypy foundation detection investigation orchestration api db tests` (159 files) all pass; `tests/orchestration/test_account_explanation.py`/`test_pattern_explanation.py` confirmed passing unmodified. **Post-implementation code review** found 10 findings (8 confirmed, 2 plausible), all fixed same session: 3 tool-catalog outputs (`similar_cases`/`path_recommendation_facts`/`relationship_graph`) weren't actually JSON-safe (raw `datetime`/enum members nested in the result — fixed via `db.repositories._audit.to_jsonable`, reused rather than re-implemented); `scripts/discover_relationships.py`/`generate_demo_data.py` were missing the `pii_hmac_secret`-configured pre-check `reconcile_relationship_hashes.py` already had (+ `.env.example` gained the missing `PII_HMAC_SECRET`/LLM-gateway lines); logging was dropped during the `llm_client`→`llm_gateway` extraction (restored, both retry-attempt and final-failure cases); a narrowed exception catch missed a plausible `{"choices": [null]}` malformed-response shape (`TypeError` added); `ToolInvoker`'s reliance on the HTTP layer for actor-to-case authorization was undisclosed (now an explicit docstring paragraph, no new enforcement code — deliberately not this tool-layer's job per Phase 10's own RBAC checklist item); `_ego_graph_tool` duplicated the shared scope-check helper inline instead of reusing its return value; `reconcile_relationship_hashes.py` did two unnecessary full-table loads and duplicated `discover_relationships.py`'s stats-printing block (both scripts now share `investigation.relationship_discovery.print_discovery_stats`); the `case_id`-cannot-be-overridden guarantee was an unenforced naming convention (`register_tool` now validates via `inspect.signature` at decoration time).
**Status:** done
