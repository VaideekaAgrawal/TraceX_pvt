...
**Verification:** all three CI gates green (ruff, mypy over the documented package list, pytest). New unit/integration coverage: `test_graph_filters.py` (pure-function filter composition + the "role differs by radius over the same seed" design-decision proof, independently recomputed), `test_customer_profile.py`, `test_transaction_search.py` (including the security-invariant test — an out-of-scope account's transactions never surface regardless of filter looseness), `test_behavior_analysis.py`, `test_timeline.py` (including the timeline↔graph `txn_id` sync-contract assertion), `test_pattern_explanation.py` (cache hit/miss, `rule_anchors` populated, zero `AiInteraction` rows on failure), plus repo-level tests for `mark_graph_expanded`/`search_for_accounts`/`count_for_accounts`, plus `test_l2_routes.py` (15 live `TestClient` checks over every new route against a seeded multi-hop source→mule→sink graph with a prior TRUE_POSITIVE_SAR case, RBAC 403/404, and the `graph_expanded` audit row). Then a live run against the real ingested dataset copy (166,207 customers / 518,889 accounts / 8,002 transactions, same throwaway-DB process Phase 5 used): trained the ensemble, ran the full detection pipeline (2,740 detections → 2,740 alerts → 20 auto-created/assigned cases), then drove every new route over `TestClient` against a real case — `GET .../graph?radius=4` returned 128 nodes / 162 edges in 325ms on a real case-linked account, profile/transaction-search/behavior/timeline all under 15ms, pattern-explanation (no LLM key configured) failed open in 6ms with zero `AiInteraction` rows persisted, evidence/note creation both 201 with the `graph_expanded` audit row confirmed written — see `docs/METRICS.md`.
**Reference:** §4.2.
**Status:** done
