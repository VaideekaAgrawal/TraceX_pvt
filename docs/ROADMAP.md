...
11. **Demo data is a first-class pitch asset, kept strictly separable.** The `DEMO-`-prefixed Phase 1B seed (KYC/PAN/phone/device/relationships + 7 golden scenarios) is what makes the Relationship Explorer, the L1 KYC surfaces, and Phase 9's recommendations demonstrable **at all** — without it the only discoverable relationships are `income_bracket`/`branch` noise at confidence 0.25–0.35 (Session 11 verified: 3,713 + 1,620 of them). Pitch framing is two-tier and **stated aloud**: *detection engine trained and validated at full scale on IBM's public AML benchmark; investigation features demonstrated on engineered scenarios with planted ground truth.* Never imply real bank data.
    ...
    **Update (Session 19, `docs/METRICS.md` §18):** the *"trained and validated at full scale on IBM's public AML benchmark"* half of that pitch line is now **literally true**, not aspirational — the full 5,078,345-row `HI-Small_Trans.csv` was ingested and the model retrained against it (Precision=0.254, Recall=0.173, F1=0.206, AUC-ROC=0.778; materially lower than the archive's previously-cited figures, which were never actually reproduced against this codebase — see §18 for why that's expected, not a regression). Separately, the KYC/transaction join that made real (non-`DEMO-`) cases undemonstrable at all (§17 Finding A) is fixed: real cases now resolve to a real customer *name* via `accounts.customer_id` (515,093 of 515,126 transaction-touched accounts, was 13). This does **not** change the "demonstrable at all" reasoning above for occupation/income/PAN richness — IBM's real dataset carries no such attributes, so full KYC depth is still `DEMO-`-exclusive. What changed is narrower but real: a real case is no longer a bare, nameless account graph.
...
- [x] **Tool layer** (`orchestration/tools/`) — a **fixed** catalog. Each tool wraps an **already-built** Phase 5–7 function (no new computation), carries a strict JSON schema (`strict: true`, `additionalProperties: false`), and returns a structured fact dict — **never prose**. `case_id` is **bound at construction, never a model-supplied argument** — the model cannot choose which case it looks at. Account-id arguments go through the existing `require_case_scoped_account`, so an out-of-case request returns a tool **error**, not data: case-scoping enforced in code, not in the prompt. Catalog (all twelve already exist): case summary · account facts · money flow (with `pct_of_total`) · filtered ego-graph · network risk · similar cases · relationships · path-recommendation facts · timeline · behavior analysis · transaction search · previous alerts.
- [x] **Grounding contract** (`orchestration/grounding.py`) — the **centrepiece of this phase, not a sub-task**. Defines the per-interaction fact bundle; the structured-output schema in which every model claim carries the fact key it came from; and the **deterministic validator** that rejects any claim whose citation does not resolve. See decision 8.
- [x] **PII egress gate** (`orchestration/redaction.py`) — a fail-closed assertion over every prompt-bound fact bundle, checked against `db/pii.py::PII_COLUMNS`. **Raises** on a PII value; never silently strips (a silent strip cannot be proven; a raise can). Populates `ai_interactions.redacted`. Belt *and* braces: the tools are separately shaped so PII isn't in the return value to begin with. Note `Relationship.value_hash` already delivers this for free — the AI sees `shared_attribute: "pan", confidence: 0.95, 3 entities` and **never the PAN**.
- [x] **`_value_hash` → HMAC-SHA256** keyed on `pii_hmac_key` — resolves Session 11's deferred item (an unsalted SHA256 of a low-entropy identifier like a PAN is brute-forceable if the DB ever leaks). Relationship rows are *derived*, so this is a regenerate via `scripts/discover_relationships.py`, **not** a migration.
...
- [x] **AI-action audit** — `facts` / `tools_called` / `rule_anchors` always-populated **by the gateway**, not per-caller. The `audit_log` hash chain already fires automatically through `BaseRepository._create` (`action="ai_interaction_created"`) — this is **not** a new mechanism, it is filling three columns that have been sitting null since Phase 1.
...
**Scope (checklist):**
- [x] **Deterministic action catalog + rule set** — `orchestration/recommendation/action_catalog.py`: 13 fixed actions, each mapped to a typology **and** a FATF+RBI/PMLA regulatory anchor (prototype-level, flagged in-code as illustrative pending compliance review). The catalog *is* the action space; `engine.py` rejects any `action_id` outside it.
- [x] **Rule grounding** — `orchestration/recommendation/rule_grounding.py`: reads a case's persisted alerts (never re-detects) into structured `RuleFiring` records (rule id, threshold tested, typology, triggering accounts, score/severity) → populates `ai_interactions.rule_anchors`. **Note:** the *threshold* is emitted here (deterministic, from the rule DSL/primitive defaults); the *observed value* comes from the tool fact bundle, grounded by the Phase-8 validator — the two are kept apart deliberately (threshold is a property of the rule, observed value a property of the data).
- [x] **Ranking policy** — `orchestration/recommendation/ranking.py`: deterministic, evidence-weighted (severity/score/breadth), computed in Python not asked of the model, structured as a weighted sum so an RL weight vector can replace the constants later.
- [x] **Reasoning + explanation** — `orchestration/agent_loop.py` (net-new shared agentic loop, reused by Phase 10): two-phase (gather via fact tools with `tool_choice=auto`, then a forced structured submission), grounded in Phase 8's tool-computed fact bundle. Added a summarised ego-graph tool (`get_ego_graph_summary`) to the Phase-8 catalog — Session 18's Finding C (full `get_ego_graph` was ~56k tokens / 2,505 facts per bundle).
- [x] **Structured output + validation gate** — `engine.py`: recommendations returned as JSON (`action_id` **must** be in the eligible catalog, `rationale` claims **must** pass the Phase-8 grounding validator, `rule_anchor` attached deterministically, confidence from ranking). Anything failing is **rejected before the investigator sees it** — a code path, not a prompt. `POST /cases/{case_id}/recommendations`.
- [x] **Cross-question dialogue** — `engine.answer_challenge` + `POST /cases/{case_id}/recommendations/challenge`: investigator challenges, engine re-invokes tools and defends with grounded cited facts, appended to the same audited `ai_interactions` thread. An answer that can't be grounded is not shown (`answered=false` + reason). Investigator free-text is length-capped; per decision 10 the threat model (authenticated investigator, output still grounded) is noted in-code.
...
**Status:** **done — merged to `main` via PR #14** (Session 23, 2026-07-17; numbered 23 not 21 to avoid a collision with the concurrent frontend Sessions 21/22). CI green on both triggers; ruff/mypy clean (CI's full invocation incl. `demo_data tests`), 33 new tests + 565 total pass. **Live verify against a real round-trip case (`docs/METRICS.md` §20):** 3 grounded FATF-anchored recommendations accepted, 1 correctly rejected for an ungrounded number — the grounding guardrail demonstrably firing on real model output; PII gate passed across 13 tool dispatches. Three real bugs found and fixed only because the verify used a live model, not mocks (fact-key prefixing, empty forced-submit from a shrunk tools list, and the actual culprit — answer-token truncation `finish_reason=length` → empty `{}`). `/code-review low`: 4 findings fixed (dead code, double rule fetch, missing route tests, challenge length-limit mismatch).
