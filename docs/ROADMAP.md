...
**Verification:** all three CI gates (ruff/mypy/pytest, Python 3.14 venv) plus a live end-to-end run against the real ingested dataset (166,207 customers / 518,889 accounts / 8,002 transactions) on a throwaway DB copy: trained the ensemble, ran the full detection pipeline (2,740 detections → 4,031 alerts → 20 auto-assigned cases), then drove every new route over `TestClient` against a real pipeline-generated case — all data-assembly endpoints, a real Network Risk Score (32.0: 2 mule accounts, 24 cycles, 3 high-centrality accounts), the AI explanation's cache round-trip, the account-scope 404, and the full decision flow (Investigator escalate → 200, Investigator close_fp → 403, Admin close_fp → 200 with a real `DetectionFeedback`+note written) — see `docs/SESSION_LOG.md` for the actual output.
**Reference:** §4.1.
**Status:** done
