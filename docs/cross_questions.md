...
Three independent measures: (1) **Temporal 70/15/15 split** — accounts are ordered by their last transaction timestamp, then split: training uses earliest 70%, validation next 15%, test final 15%. This simulates production exactly — the model only ever sees past data during training. Random splits on time-series data consistently overestimate model performance by 15–30% AUC by allowing future information to leak into training. (2) **Source-only labelling** — only the source account of a confirmed laundering transaction is labelled positive, because innocent recipients of laundering transactions would otherwise inflate label noise (the archive's own documented ablation found including destination accounts dropped precision from 77.8% to 4.9%; not independently re-run against this codebase's full-scale ingest). (3) **PR-curve threshold optimisation on validation set** — after training, the classification threshold is tuned on the held-out validation set using `precision_recall_curve()`, not the test set. The test set is only touched once, for final evaluation. These three measures are what this codebase's `FraudClassifier.train()` actually implements; run for real against the full IBM HI-Small benchmark (5,078,345 transactions, temporal split, threshold 0.4977), the measured metrics are Precision=0.254, Recall=0.173, F1=0.206, AUC-ROC=0.778 (`docs/METRICS.md` §18) — materially lower than the archive's previously-cited 0.778/0.609/0.683/0.933, because those were never reproduced against this codebase's actual training code at any scale, and the real task is a genuinely hard 0.48%-positive-rate classification problem.
...
Measured for real against the full IBM HI-Small benchmark (`docs/METRICS.md` §18, model_run `RUN-CC8AD2C6344F4525`, 2026-07-15) — every one of the 5,078,345 real transactions ingested, 15% temporal holdout, PR-curve-optimised threshold: **Precision=0.254 (25.4%), Recall=0.173 (17.3%), F1=0.206, AUC-ROC=0.778**, at optimal threshold 0.4977. Test set: 74,550 accounts (positive rate 0.48% — 1,666 of 347,896 training-set accounts). Training took 7.4s on GPU/CUDA. These numbers are honestly lower than the archive's previously-cited Precision=0.778/F1=0.683/AUC=0.933 (`archive/fund-flow-tracker/infrastructure/config.py`'s documented tuning result) — that figure was never reproduced against this codebase's actual ingest/feature/training code at any scale, and the real, full-scale, 0.48%-positive-rate account classification task is genuinely hard (extreme class imbalance is a known characteristic of this specific IBM benchmark, not a defect in this run). AUC-ROC=0.778 shows the model has real discriminative power despite the low precision/recall at this operating point. The `fraud_metrics` dict returned by `FraudClassifier.train()` includes: precision, recall, f1, precision_default, recall_default, auc_roc, confusion_matrix, train_size, val_size, test_size, positive_rate, training_time_sec, device (GPU/CPU), optimal_threshold, and best_iteration. All of this is live-queryable at `/api/model-metrics` during the demo.
