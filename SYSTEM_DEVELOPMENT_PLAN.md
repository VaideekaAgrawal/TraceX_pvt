...
- **Metric inconsistency:** `README.md` states XGBoost F1=0.683, AUC-ROC=0.933; `docs/cross_questions.md` states F1≈0.72, AUC-ROC≈0.88. These need to be reconciled before a judge who reads both documents notices. **Resolved in ROADMAP Phase 3:** `docs/cross_questions.md`'s figures corrected to match the README/archive's documented tuning result (PR-AUC=0.64, Precision=0.778, Recall=0.609, F1=0.683, CV AUC=0.933).
