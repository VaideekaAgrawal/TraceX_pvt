...
and any user-defined rule compose, replacing the hardcoded 6-detector list
in DetectionService with a DB-backed registry.
...
Two tiers of rule:
  Tier 1 — a rule with exactly one, non-negated condition. Its primitive's
           ...
           byte-for-byte. This is also how editing a threshold (e.g.
           round-trip's return ratio 0.85 -> 0.70) takes effect with no
           code change: the primitive is re-instantiated with the rule's
           current params on every pipeline run.
...
Residual limits (see docs/systemexplanation or the plan): a graph-topology
shape outside this primitive catalog still needs a new Python primitive
here — now an isolated addition rather than touching detector, ensemble,
and evidence code. ML-model signals (IsolationForest/XGBoost) aren't
expressible in this DSL. Composition is account-level only — no true
cross-primitive topological joins.
"""
import logging
...
class PrimitiveRegistry:
    ...
    SCHEMAS: Dict[str, Dict[str, str]] = {
        ...
        "chain": {"min_hops": "int", "time_window_minutes": "int", "extended_min_hops": "int",
                  "extended_window_minutes": "int", "decay_ratio_threshold": "float"},
        "fan_degree": {"direction": "enum:fan_out,fan_in", "min_degree": "int", "window_days": "int"},
        "bipartite_scatter_gather": {"min_side": "int", "window_days": "int"},
        "amount_band_count": {"lower": "float", "upper": "float", "min_count": "int", "window_days": "int"},
        "split_sum_threshold": {"lower": "float", "upper": "float", "min_count": "int"},
        "inactivity_then_burst": {"threshold_days": "int", "min_burst_txns": "int", "multiplier": "float"},
        "volume_vs_declared_income_ratio": {"ratio_threshold": "float"},
        "peer_zscore_deviation": {"z_threshold": "float", "min_peer_group_size": "int"},
        "behavioural_shift": {"shift_z_threshold": "float", "min_txn_count": "int", "rolling_window": "int"},
        "generic_group_aggregate": {"group_by": "enum:source_account,dest_account", "agg": "enum:SUM,COUNT,AVG,MAX,MIN",
    ...
    DEFAULTS: Dict[str, Dict[str, Any]] = {
        ...
        "amount_band_count": {"lower": 900_000, "upper": 1_000_000, "min_count": 3, "window_days": 30},
        "split_sum_threshold": {"lower": 900_000, "upper": 1_000_000, "min_count": 2},
        "inactivity_then_burst": {"threshold_days": 180, "min_burst_txns": 5, "multiplier": 10.0},
    ...
    # Sane ceilings on params that drive expensive graph traversals — mirrors
    # the existing _MAX_PER_TYPE/max_cycles/max_chains safety valves.
    ...
    @staticmethod
    def evaluate(primitive: str, params: Dict[str, Any], graph_engine,
                 accounts_df: Optional[pd.DataFrame],
                 transactions_df: pd.DataFrame) -> List[DetectionResult]:
        if primitive == "cycle":
            return RoundTripDetector(params).detect(graph_engine, transactions_df)

        if primitive == "chain":
            return LayeringDetector(params).detect(graph_engine, transactions_df)
        ...
        if primitive == "fan_degree":
            ...
            if direction == "fan_in":
                return det._detect_fan(df, group_col="dest_account", target_col="source_account",
                                        min_fan=det.cfg.fan_out_min_degree, window_ns=window_ns, direction="fan_in")
            return det._detect_fan(df, group_col="source_account", target_col="dest_account",
        ...
        if primitive == "bipartite_scatter_gather":
            ...
            df = _sorted_by_time(transactions_df)
            window_ns = int(det.cfg.fan_out_time_window_days * 86_400 * 1e9)
            return det._detect_bipartite(df, min_side=det.cfg.bipartite_min_side, window_ns=window_ns)
        ...
        if primitive == "inactivity_then_burst":
            return DormancyDetector(params).detect(graph_engine, transactions_df)

        if primitive == "volume_vs_declared_income_ratio":
            if accounts_df is None:
                return []
            return ProfileMismatchDetector(params)._detect_income_mismatch(transactions_df, accounts_df)

        if primitive == "peer_zscore_deviation":
            if accounts_df is None:
                return []
            return ProfileMismatchDetector(params)._detect_peer_deviation(transactions_df, accounts_df)
...
def _generic_group_aggregate(params: Dict[str, Any], transactions_df: pd.DataFrame) -> List[DetectionResult]:
    ...
    for acc_id, agg_val in matched.items():
        score = round(min(1.0, 0.5 + min(abs(float(agg_val) - value) / max(abs(value), 1.0), 0.5)), 3)
        results.append(DetectionResult(
            ...
            indicators=[f"{agg}({field}) over {window_days}d {operator} {value:,.0f}: actual={agg_val:,.2f}"],
        ))
    return results
...
class RuleEvaluator:
    ...
    @classmethod
    def evaluate_rule(cls, rule: Dict[str, Any], graph_engine,
                      accounts_df: Optional[pd.DataFrame],
                      transactions_df: pd.DataFrame) -> List[DetectionResult]:
        ...
        if len(conditions) == 1 and not conditions[0].get("negate", False):
            ...
            results = PrimitiveRegistry.evaluate(
                cond["primitive"], cond.get("params", {}), graph_engine, accounts_df, transactions_df,
            )
            for r in results:
                r.detection_type = rule["detection_type"]
            return results

        return cls._evaluate_composite(rule, combinator, conditions, graph_engine, accounts_df, transactions_df)

    @classmethod
    def _evaluate_composite(cls, rule, combinator, conditions, graph_engine, accounts_df, transactions_df):
...
class RuleEngine:
    ...
    single call DetectionService.run_full_pipeline() makes in place of the
    old hardcoded 6-detector-instance list."""

    def run_all(self, graph_engine, accounts_df: Optional[pd.DataFrame],
               transactions_df: pd.DataFrame) -> Dict[str, List[DetectionResult]]:
        from infrastructure.database import get_database

        rules = get_database().list_rules(enabled_only=True)
        grouped: Dict[str, List[DetectionResult]] = {}
        for rule in rules:
            try:
                results = RuleEvaluator.evaluate_rule(rule, graph_engine, accounts_df, transactions_df)
            except Exception:
                logger.exception("Rule %s ('%s') failed to evaluate — skipping",
                                 rule.get("rule_id"), rule.get("name"))
                continue
            grouped.setdefault(rule["detection_type"], []).extend(results)
        return grouped

    def dry_run(self, rule_json: Dict[str, Any], detection_type: str, severity: str,
               graph_engine, accounts_df: Optional[pd.DataFrame],
        ...
        results = RuleEvaluator.evaluate_rule(fake_rule, graph_engine, accounts_df, transactions_df)
        matched_accounts = sorted(set(a for r in results for a in r.account_ids))
        return {
