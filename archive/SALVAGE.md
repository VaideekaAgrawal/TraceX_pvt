...
|---|---|---|---|
| ML ensemble (IsolationForest + XGBoost) | `services/detection/ensemble.py` **+ `infrastructure/config.py`** (`DetectionConfig`, lines ~52-66) | **Port the training/scoring logic, not a saved artifact — none exists.** XGBoost is trained on real labelled data at pipeline-run time (`gpu_hist` if CUDA available), not loaded from a serialized file. See "Important note on the trained model" below. **Also port the calibrated hyperparameters from `infrastructure/config.py`'s `DetectionConfig` — do not re-derive them from library defaults.** They are the result of a documented tuning experiment ("best config: capped_spw, exp v2, 2026-05-18"): `xgb_scale_pos_weight` deliberately capped at `15.0` (auto-tuning produced ~80, which cratered precision to 4.9%), plus `xgb_n_estimators=500`, `max_depth=6`, `learning_rate=0.03`, `min_child_weight=5`, `subsample=0.8`, `colsample_bytree=0.7`, `gamma=2.0`, `reg_alpha=0.5`, `reg_lambda=2.0`, `early_stopping_rounds=50`, `optimal_threshold=0.5` (set post-training via a PR curve on the validation set), `label_mode="source_only"`. Documented result at these settings: PR-AUC=0.64, Precision=0.778, Recall=0.609, F1=0.683, CV AUC=0.933 — this is the number set to reconcile against the README/cross_questions discrepancy noted in ROADMAP Phase 3. | `backend/detection/scoring/ensemble.py` behind a `Scorer` interface |
