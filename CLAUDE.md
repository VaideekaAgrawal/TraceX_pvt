...
- Before opening a PR / merging a phase branch to `main`: run **`/code-review low`** (and `/verify` if the change has a runtime surface) on the full phase diff. **Effort tiering (explicit user direction, Session 19): `low` by default, `medium` only if `low` surfaces something that needs deeper follow-up or the diff genuinely warrants it, `high` reserved specifically for the backend-completion milestone review** (i.e. not per-phase — a single comprehensive pass once the backend phases are done), never `xhigh`/`ultra` without the user asking by name. Every prior session that left the effort level unspecified independently defaulted to `high` (8 parallel finder-angle agents + a verify pass) at every single phase merge — reasonable in isolation, but that cost compounded across a 20+ session refactor and burned session/weekly budget fast enough that the user flagged it explicitly.
